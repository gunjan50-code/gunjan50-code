...
from config import ROOT, prompt
...
# Reveal timing (one-shot). Slow enough to actually be watched: on a sparse
# graph nearly every cell is the empty colour, which sits a few RGB points off
# the panel background, so a fast subtle fade looks like nothing happened at
# all. The scale pop is what makes the sweep visible on an empty grid; the
# brightness flash is what makes the contributed days stand out from it.
...
@keyframes cell {{
  ...
  62%  {{ opacity: 1; transform: scale(1.20); }}
...
.g {{
  animation: cell {CELL_DUR:.2f}s cubic-bezier(.2,.8,.2,1) both,
             flash {CELL_DUR + 0.25:.2f}s ease-out both;
}}
@media (prefers-reduced-motion: reduce) {{
  .c, .g {{ opacity: 1; animation: none; filter: none; transform: none; }}
}}
""".strip()
    ...
    for ci, column in enumerate(grid):
        ...
        for ri, cell in enumerate(column):
            ...
            cls = "c g" if lvl else "c"      # days with contributions also flash
            parts.append(
