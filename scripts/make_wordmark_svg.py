"""
Render your initials as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it
as an SVG that animates on GitHub (SMIL only -- GitHub runs SVG animations in
<img>, but never JS).
...
from config import ROOT, WORDMARK_TEXT, prompt, BG, BG2, FRAME, TITLE_TEXT, INK
...
# An even stroke weight keeps the shading consistent across a letter, and a
# pointed apex (A, V, W) gives the extrusion an edge to wrap around. A very
# heavy face (Impact, Arial Black) collapses to blobs at this resolution -- the
# counters get thinner than one grid cell. Futura Bold on macOS is the original;
# Arial Bold is the closest thing shipped with Windows.
FONT_CANDIDATES = [
    (os.environ.get("WORDMARK_FONT"), int(os.environ.get("WORDMARK_FONT_INDEX", 0))),
    ("/System/Library/Fonts/Futura.ttc", 2),                 # macOS
    ("C:/Windows/Fonts/arialbd.ttf", 0),                     # Windows
    ("C:/Windows/Fonts/trebucbd.ttf", 0),
...
def resolve_font():
    for path, index in FONT_CANDIDATES:
        if path and os.path.exists(path):
            return path, index
    print("no bold TTF found -- set WORDMARK_FONT to one", file=sys.stderr)
    sys.exit(1)
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">{html.escape(prompt("~$ ./wordmark.sh --3d"))}</text>')
...
def main():
    ...
    out = a.out or os.path.join(ROOT, "wordmark.svg")
