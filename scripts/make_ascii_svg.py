...
portrait prints once and freezes.
    ...
    python scripts/make_ascii_svg.py [source-prepped.png] [portrait-ascii.svg]
"""
...
from config import ROOT, DISPLAY_NAME, SHELL_USER, prompt, BG, BG2, FRAME, TITLE_TEXT, INK
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(ROOT, "portrait-ascii.svg")
...
# light global tuning is needed here. Tweak these three if your photo comes out
# too dark (raise GAMMA) or too washed out (lower WHITE_FLOOR).
CONTRAST = 1.0        # prep_photo.py already stretched the range; don't re-crush it
BRIGHTNESS = 1.0
GAMMA = 0.85          # <1 lifts the midtones into sparser chars, >1 darkens them
SHARPEN = False
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{html.escape(prompt("~$ ./portrait.sh"))}</text>')
...
# status bar with a steady blinking cursor. the cursor x is derived from the
# rendered text width (monospace advance ~= 0.6em) so it lands right after the
# name no matter how long the name is.
status_prefix = f"{SHELL_USER}@github:~$ whoami "
status_line_y = TITLEBAR_H + ART_H + PAD * 0.35
status_y = status_line_y + 19
cursor_x = PAD + (len(status_prefix) + len(DISPLAY_NAME) + 0.5) * 13 * 0.6
